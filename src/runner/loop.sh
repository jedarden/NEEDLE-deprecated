...
NEEDLE_LOOP_SHUTDOWN_GRACE_PERIOD="5s"
# ============================================================================
# Signal Handlers
# ============================================================================
...
_needle_worker_loop() {
    ...
    polling_interval=$(get_config "runner.polling_interval" "$NEEDLE_LOOP_DEFAULT_POLLING_INTERVAL")
    local idle_timeout
    idle_timeout=$(get_config "runner.idle_timeout" "$NEEDLE_LOOP_DEFAULT_IDLE_TIMEOUT")
    # Remove 's' suffix
    polling_interval="${polling_interval%s}"
    idle_timeout="${idle_timeout%s}"
    local max_consecutive_empty
    max_consecutive_empty=$(get_config "runner.max_consecutive_empty" "$NEEDLE_LOOP_DEFAULT_MAX_EMPTY")
    ...
    while true; do
        ...
        if [[ "$_NEEDLE_LOOP_SHUTDOWN" == "true" ]]; then
            ...
            _needle_info "Allowing $grace_period seconds for cleanup..."
            sleep "$grace_period"
            _needle_emit_event "worker.stopped" "reason=shutdown"
            break
        fi
        ...
        if _needle_strand_engine "$workspace" "$agent"; then
            ...
            if [[ $strand_result -eq 0 ]]; then
                ...
                _needle_emit_event "worker.idle" \
                ...
                if ((idle_seconds >= idle_timeout)); then
                    ...
                    _needle_emit_event "worker.stopped" "reason=idle_timeout"
...
# ============================================================================
# Fail a bead and releasing it back to queue
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    source "$(dirname "${BASH_SOURCE[0]}")/../telemetry/writer.sh"
    source "$(dirname "${BASH_SOURCE[0]}")/../watchdog/heartbeat.sh"
    source "$(dirname "${BASH_SOURCE[0]}")/../runner/state.sh"
    source "$(dirname "${BASH_SOURCE[0]}")/../hooks/runner.sh"
    source "$(dirname "${BASH_SOURCE[0]}")/../strands/engine.sh"
    ...
    case "${1:-}" in
        ...
        process)
            ...
            NEEDLE_MODEL="sleep"
            NEEDLE_IDENTIFIER="test"
            export NEEDLE_SESSION NEEDLE_RUNNER NEEDER_PROVIDER NEEDLE_MODEL NEEDLE_IDENTIFIER
