...
_needle_build_prompt() {
    ...
    # Output format: "Labels for <id>:\n  label1\n  label2" — strip header and indent, join with ", "
    labels=$(cd "$workspace" && br label list "$bead_id" --no-color 2>/dev/null | sed '1d' | sed 's/^  //' | paste -sd, - | sed 's/,$//' | sed 's/,/, /g')
